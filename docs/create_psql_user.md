...
按 `docs/PSQL_Account.md` 的边界创建四组账号，并让每个账号只能访问自己的数据库。
...
```bash
docker exec -i mem012-postgres psql -v ON_ERROR_STOP=1 -U utest -d TESTD <<'SQL'
SELECT format('CREATE DATABASE %I OWNER utest', db_name)
FROM (VALUES
  ('mem_riko'),
  ('mem_herm'),
  ('mem_doge'),
...
ALTER DATABASE mem_riko OWNER TO utest;
ALTER DATABASE mem_herm OWNER TO utest;
ALTER DATABASE mem_doge OWNER TO utest;
ALTER DATABASE mem_share OWNER TO utest;
SQL
```
...
SELECT format('REVOKE ALL PRIVILEGES ON DATABASE %I FROM riko, herm, doge, "share"', datname)
FROM pg_database
WHERE datallowconn AND NOT datistemplate\gexec
...
ALTER SCHEMA public OWNER TO utest;
REVOKE ALL ON SCHEMA public FROM PUBLIC;
REVOKE ALL ON SCHEMA public FROM riko, herm, doge, "share";
REVOKE ALL PRIVILEGES ON ALL TABLES IN SCHEMA public FROM riko, herm, doge, "share";
REVOKE ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public FROM riko, herm, doge, "share";
REVOKE ALL PRIVILEGES ON ALL FUNCTIONS IN SCHEMA public FROM riko, herm, doge, "share";
ALTER DEFAULT PRIVILEGES FOR ROLE utest IN SCHEMA public REVOKE ALL ON TABLES FROM riko, herm, doge, "share";
ALTER DEFAULT PRIVILEGES FOR ROLE utest IN SCHEMA public REVOKE ALL ON SEQUENCES FROM riko, herm, doge, "share";
...
ALTER DEFAULT PRIVILEGES FOR ROLE utest IN SCHEMA public GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO riko;
ALTER DEFAULT PRIVILEGES FOR ROLE utest IN SCHEMA public GRANT USAGE, SELECT, UPDATE ON SEQUENCES TO riko;
...
ALTER DEFAULT PRIVILEGES FOR ROLE utest IN SCHEMA public GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO herm;
ALTER DEFAULT PRIVILEGES FOR ROLE utest IN SCHEMA public GRANT USAGE, SELECT, UPDATE ON SEQUENCES TO herm;
...
ALTER DEFAULT PRIVILEGES FOR ROLE utest IN SCHEMA public GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO doge;
ALTER DEFAULT PRIVILEGES FOR ROLE utest IN SCHEMA public GRANT USAGE, SELECT, UPDATE ON SEQUENCES TO doge;
SQL
```
...
```bash
docker exec -i mem012-postgres psql -v ON_ERROR_STOP=1 -U utest -d mem_share <<'SQL'
CREATE EXTENSION IF NOT EXISTS vector;
CREATE EXTENSION IF NOT EXISTS pg_trgm;
CREATE EXTENSION IF NOT EXISTS age;
...
ALTER DEFAULT PRIVILEGES FOR ROLE utest IN SCHEMA public GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO "share";
ALTER DEFAULT PRIVILEGES FOR ROLE utest IN SCHEMA public GRANT USAGE, SELECT, UPDATE ON SEQUENCES TO "share";
SQL
```
...
```bash
docker exec mem012-postgres psql -U utest -d TESTD -Atc \
  "select rolname, rolsuper, rolcreatedb, rolcreaterole, rolreplication from pg_roles where rolname in ('riko','herm','doge','share') order by rolname;"

docker exec mem012-postgres psql -U utest -d TESTD -Atc \
  "select member::regrole || '->' || roleid::regrole from pg_auth_members where member in ('riko'::regrole,'herm'::regrole,'doge'::regrole,'share'::regrole);"
```
...
for db in mem_riko mem_herm mem_doge mem_share; do
  psql -U utest -d "$db" -Atc "select current_database() || chr(58) || string_agg(extname, chr(44) order by extname) from pg_extension where extname in (chr(97)||chr(103)||chr(101), chr(112)||chr(103)||chr(95)||chr(116)||chr(114)||chr(103)||chr(109), chr(118)||chr(101)||chr(99)||chr(116)||chr(111)||chr(114));"
done'
```
...
docker exec -e PGPASSWORD="$MEM012_PSQL_USER_PASSWORD" mem012-postgres sh -lc '
set -eu
for spec in riko:mem_riko herm:mem_herm doge:mem_doge share:mem_share; do
  user=${spec%%:*}
  db=${spec#*:}
