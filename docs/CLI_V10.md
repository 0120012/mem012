...
创建记忆提案。该工具只写入 `memory_changes`，不直接写入正式表。
...
- 本调用返回后，记忆处于 `pending_review` 状态
- `memory_units`、`memory_keywords`、`memory_handles`、`memory_relations` 仍不变
- `memory_uuid` 由后端生成，调用方不得传入
- `title` 只作为输入字段，后端必须调用数据库 `normalize_title(text)` 得到 `title_norm`
- `memory_changes.after_state` 保存 `title_norm`，不保存 `title`
